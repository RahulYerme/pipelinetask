...
pipeline {
  ...
  stage("docker_scan"){
   steps{
        ...
        sh '''
         ...
         ./clair-scanner --ip="$DOCKER_GATEWAY" --threshold="High" rahulyerme1234/jenkinspipeline:57 || exit 0
       '''
    }
   }
  }	  
 ...
 post {
    success {
      office365ConnectorSend color: '#00cc00', message: "Success  ${env.JOB_NAME} build_number:${env.BUILD_NUMBER}, branch:${env.GIT_BRANCH} url:(<${env.BUILD_URL}>)", status: 'SUCCESS', webhookUrl: "${TEAMS_WEBHOOK}"
    }
    failure {
